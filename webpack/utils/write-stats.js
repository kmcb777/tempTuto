...
const filepath = path.resolve(__dirname, "../../tmp/webpack-stats.json");
...
function writeStats(stats) {
  ...
  let frontScripts = getChunks("front", "js");
  let adminScripts = getChunks("admin", "js");

  if (process.env.LOCAL) {
    frontScripts.push(getChunks("evil-icons", "js"));
    adminScripts.push(getChunks("evil-icons", "js"));
  }

  const content = {
    front: {
      script: frontScripts,
      css: getChunks("front", "css")
    },
    admin : {
      script: adminScripts,
      css: getChunks("admin", "css")
    }
